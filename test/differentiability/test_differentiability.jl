# Differentiability tests for Lambert.jl
#
# Validates that AD backends produce correct Jacobians of solve() w.r.t. all
# inputs (μ, r₁, r₂, tof) for each Lambert solver. Also validates the
# analytical Jacobian from lambert_jacobian() against ForwardDiff.
#
# This file expects:
#   _BACKENDS  — Tuple of (name::String, backend) pairs (defined in runtests.jl)
#   DifferentiationInterface, FiniteDiff, ForwardDiff — already loaded
...
# Solvers to test with AD (all that support ForwardDiff)
const _DIFF_SOLVERS = [
    ("Izzo", IzzoSolver()),
...
# Solvers that use Roots.jl internally (not AD-compatible), validated via FiniteDiff
...
@testset "Analytical Jacobian vs ForwardDiff" begin
    ...
    # bisection). Cross-check: their STM should match a high-accuracy solver's STM since
    # both solve the same Lambert problem to similar accuracy.
    J_ref_izzo = let
        prob = LambertProblem(_DIFF_MU, _DIFF_R1, _DIFF_R2, _DIFF_TOF)
...
@testset "AD Backends" begin
    for (bname, backend) in _BACKENDS
        @testset "$bname" begin
            for (sname, solver) in _DIFF_SOLVERS
                @testset "$sname" begin
                    J_ref = jacobian(x -> _lambert_map(x, solver), _REF_BACKEND, _DIFF_X0)
