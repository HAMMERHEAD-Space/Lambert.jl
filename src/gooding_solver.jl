...
function SciMLBase.solve(problem::LambertProblem, solver::GoodingSolver)
    ...
    v1, v2, numiter, retcode = gooding1990(μ, r1, r2, tof; M=M, prograde=prograde, low_path=low_path, maxiter=maxiter, atol=atol, rtol=rtol)
...
function gooding1990(
    ...
    (M > 0) && error("Multi-revolution case not implemented - requires significant algorithm extension")
    ...
    vr1, vt1, vr2, vt2, numiter = vlamb(
        μ, r1_norm, r2_norm, dtheta, tof, low_path, maxiter, atol, rtol
    )
    ...
    v1, v2 = reconstruct_velocities_from_components(vr1, vt1, vr2, vt2, i_r1, i_t1, i_r2, i_t2)
...
function tlamb(m::Int, q::Number, qsqfm1::Number, x::Number, n::Int)
    ...
    if lm1 || m > 0 || x < 0.0 || abs(u) > sw
        ...
        while i < n || t != told
            ...
            if l1 && i > 1; u1i = u1i * u; end
            if l2 && i > 2; u2i = u2i * u; end
            if l3 && i > 3; u3i = u3i * u; end
            ...
            if l1; dt = dt + tqterm * u1i; end
            if l2; d2t = d2t + tqterm * u2i * (p - 1.0); end
            if l3; d3t = d3t + tqterm * u3i * (p - 1.0) * (p - 2.0); end
        end
        
        if l3; d3t = 8.0 * x * (1.5 * d2t - xsq * d3t); end
        if l2; d2t = 2.0 * (2.0 * xsq * d2t - dt); end
        if l1; dt = -2.0 * x * dt; end
...
"""
function vlamb(
    μ::Number, r1_norm::Number, r2_norm::Number, dtheta::Number, tof::Number, low_path::Bool, maxiter::Int, atol::Float64, rtol::Float64
)
...
"""
function xlamb(m::Int, q::Number, qsqfm1::Number, tin::Number, maxiter::Int, atol::Float64, rtol::Float64)
