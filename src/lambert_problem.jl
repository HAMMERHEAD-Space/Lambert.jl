...
function LambertProblem(μ::Number, coord1::AstroCoords.AstroCoord, coord2::AstroCoords.AstroCoord, tof::Number)
...
function SciMLBase.remake(prob::LambertProblem;
                         μ=nothing,
                         r1=nothing,
                         r2=nothing,
                         tof=nothing)
...
function SciMLBase.init(prob::LambertProblem; alg=nothing, M::Int=0, prograde::Bool=true, kwargs...)
    if alg === nothing
...
function solve(prob::LambertProblem; alg=nothing, M::Int=0, prograde::Bool=true, kwargs...)
    return SciMLBase.solve!(SciMLBase.init(prob; alg=alg, M=M, prograde=prograde, kwargs...))
end
