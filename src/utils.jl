...
"""
function assert_parameters_are_valid(μ::Number, r1::Vector{<:Number}, r2::Vector{<:Number}, tof::Number, M::Int)
...
"""
@inline function check_convergence(x_new::Number, x_old::Number, atol::Float64, rtol::Float64)
    return abs(x_new - x_old) <= atol || abs(x_new - x_old) <= rtol * max(abs(x_new), abs(x_old))
end
...
Returns true if converged.
"""
@inline function check_convergence(vec_new::Vector{<:Number}, vec_old::Vector{<:Number}, atol::Float64, rtol::Float64)
...
"""
function create_lambert_solution(v1::Union{AbstractVector, Nothing}, v2::Union{AbstractVector, Nothing}, numiter::Int, retcode::Symbol)
...
"""
@inline function compute_unit_vectors(r1::Vector{<:Number}, r2::Vector{<:Number}, r1_norm::Number, r2_norm::Number)
    i_r1 = r1 / r1_norm
    i_r2 = r2 / r2_norm  
...
Returns (i_t1, i_t2).
"""
@inline function compute_tangential_unit_vectors(i_h::Vector{<:Number}, i_r1::Vector{<:Number}, i_r2::Vector{<:Number})
...
@inline function reconstruct_velocities_from_components(
    vr1::Number, vt1::Number, vr2::Number, vt2::Number,
    i_r1::AbstractVector{<:Number}, i_t1::AbstractVector{<:Number}, 
    i_r2::AbstractVector{<:Number}, i_t2::AbstractVector{<:Number}
)
...
Returns (v1, v2).
"""
@inline function reconstruct_velocities_fg(f::Number, g::Number, gdot::Number, r1::AbstractVector{<:Number}, r2::AbstractVector{<:Number})
    r1 = SVector{3}(r1[1], r1[2], r1[3])
    r2 = SVector{3}(r2[1], r2[2], r2[3])
...
"""
function compute_unit_vector_geometry(r1::AbstractVector{<:Number}, r2::AbstractVector{<:Number}, 
                                     r1_norm::Number, r2_norm::Number)
    i_r1 = r1 / r1_norm
    i_r2 = r2 / r2_norm
    i_h = cross(i_r1, i_r2)
    i_h_norm = norm(i_h)
...
"""
function universal_anomaly_initial_guess(μ::Number, tof::Number, s::Number, M::Int, tof_parabolic::Number)
    if M == 0
...
"""
@inline function apply_damping(delta::Number, current_value::Number; max_relative_change::Float64=0.5)
...
"""
function validate_lagrange_coefficients(f::Number, g::Number, gdot::Number; tolerance::Float64=1e-10)
...
"""
function select_lambert_algorithm(problem::LambertProblem, M::Int=0, prograde::Bool=true)
...
"""
function generic_solver_solve(problem::LambertProblem, solver::AbstractLambertSolver, algorithm_function::Function)
...
"""
function normalized_time_of_flight_function(x::Number, target_tof::Number, 
...
"""
function transfer_classification(r1::AbstractVector{<:Number}, r2::AbstractVector{<:Number}, 
                                prograde::Bool, tof::Number, μ::Number)
...
"""
function normalize_inputs(μ::Number, r1::AbstractVector{<:Number}, r2::AbstractVector{<:Number}, tof::Number)
...
"""
@inline function denormalize_velocities(v1_hat::AbstractVector{<:Number}, v2_hat::AbstractVector{<:Number}, 
                                       L_ref::Number, T_ref::Number)
...
"""
function compute_lagrange_coefficients_universal(χ::Number, r1_norm::Number, r2_norm::Number, 
                                               μ::Number, α::Number, ψ::Number)
    C = c2(ψ)
    S = c3(ψ)
...
"""
@inline function robust_division(numerator::Number, denominator::Number; 
                                threshold::Float64=1e-15, fallback::Number=0.0)
