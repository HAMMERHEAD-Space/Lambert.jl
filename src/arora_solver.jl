...
function SciMLBase.solve(problem::LambertProblem, solver::AroraSolver)
    ...
    v1, v2, numiter, retcode = arora2013(μ, r1, r2, tof; M=M, prograde=prograde, maxiter=maxiter, atol=atol, ε=ε)
...
function arora2013(
    ...
    if tof_hat <= tof_p
        ...
        elseif (d == -1) && (tof_hat <= tof20)
            ...
            t_0 = tof20
            t_1 = tof100
            k = ((t_1 * (t_0 - t_star) * 10 - t_0 * √20 * (t_1 - t_star)) / 
                 (t_star * (t_0 - t_1)))^2
        end
    else
        ...
        if M == 0
            ...
            W_set = [4839.684497246, 212.087279879, 5.712388981, 
                     1.954946607, 1.110720735, 0.6686397730]
            ...
            elseif tof_m1 <= tof_hat <= tof_m138
                ...
                k = -c4 * (((γ1 * c1 - c3 * γ3) * c2 + c3 * c1 * γ2) / 
                           (γ3 * c1 - c3 * γ1 - γ2 * c2))^(1/α)
...
@inline function get_x(
    ...
    x = ((Z * (F_0 - F_star) * (F_1 - F_i)) / 
         ((F_i - F_star) * (F_1 - F_0) * Z + (F_0 - F_i) * (F_1 - F_star)))^(1/α)
    return x
end
...
function get_W(
    ...
    elseif sq2 - ε <= k <= sq2 + ε
        ...
        v = k - sq2
        v2, v3, v4, v5, v6, v7, v8 = [v^i for i in 2:8]
        
        W = (√2 / 3) - (1 / 5) * v + (2 / 35) * sq2 * v2 - (2 / 63) * v3 +
...
function get_Wprime(
    ...
    elseif √2 - ε < k < √2 + ε
        ...
        sq2 = √2
        v = k - sq2
        v2, v3, v4, v5, v6, v7 = [v^i for i in 2:7]
        
        W_prime = -1/5 + sq2 * v * (4/35) - v2 * (6/63) + sq2 * v3 * (8/231) -
                  v4 * (10/429) + sq2 * v5 * (48/6435) - v6 * (56/12155) +
                  sq2 * v7 * (64/46189)
    else
        W_prime = (-2 + 3 * W * k) / m
    end
...
function get_W2prime(
    ...
    elseif √2 - ε < k < √2 + ε
        ...
        sq2 = √2
        v = k - sq2
        v2, v3, v4, v5, v6 = [v^i for i in 2:6]
        
        W_2prime = sq2 * (4/35) - v * (12/63) + sq2 * v2 * (24/231) -
                   v3 * (40/429) + sq2 * v4 * (240/6435) - v5 * (336/12155) +
                   sq2 * v6 * (448/46189)
    else
        W_2prime = (5 * W_prime * k + 3 * W) / m
    end
